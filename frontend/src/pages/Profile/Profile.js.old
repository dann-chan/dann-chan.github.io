...
const ProfilePage = ({ secretData, onLogout }) => {
  const [activeTab, setActiveTab] = useState('biography'); // State switcher to track active tabs
  ...
  // Dynamic Workspace Render function mapping switch states to right-side elements
  const renderMainContent = () => {
    switch (activeTab) {
      case 'resume':
        return (
          <div className="tab-placeholder-view">
            <h2>Resume View Frame</h2>
            <p>Your timeline summaries, career matrices, or interactive elements go here.</p>
          </div>
        );
      case 'projects':
        return (
      ...
      default:
        return (
          <>
            ...
            {/* Main Content Details Grid Layout */}
  ...
  return (
    <div className="app-workspace-layout">
      ...
      <nav className="app-navigation-sidebar">
        ...
        {/* 🚪 Secure Sign Out Action trigger anchored safely at bottom row grid position */}
        <button className="sidebar-logout-btn" onClick={onLogout}>
          Secure Sign Out
        </button>
      </nav>

      {/* 🖥️ Right Column Content Viewframe Container */}
